#!/usr/bin/env bash
# eval/run_all_eval.sh — 一键串行跑所有方法并打印对比表格
# 请从项目根目录执行：bash eval/run_all_eval.sh
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

echo "======================================================"
echo "  EDAAgentMemory — 全量评估"
echo "  NavieRAG → GraphRAG Local → GraphRAG Global → TPMA"
echo "======================================================"

echo ">>> [1/4] NavieRAG..."
bash "${SCRIPT_DIR}/naive_rag/run_eval.sh"

echo ">>> [2/4] GraphRAG Local..."
bash "${SCRIPT_DIR}/graphrag/run_local_eval.sh"

echo ">>> [3/4] GraphRAG Global..."
bash "${SCRIPT_DIR}/graphrag/run_global_eval.sh"

echo ">>> [4/4] TPMA (Ours)..."
bash "${SCRIPT_DIR}/tpma/run_eval.sh"
...
files = {
    ...
    "TPMA (Ours)":     base / "tpma/results.json",
}
metrics = ["bleu-1","bleu-2","bleu-3","bleu-4","rouge_l","bert_score_f1","recall@k"]
print(f"{'Method':<22}" + "".join(f"{m:>14}" for m in metrics))
